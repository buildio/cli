...
- **Build Process**: Uses Docker with Alpine Linux for the static Linux binary, and MacPorts-hosted dependencies on GitHub macOS runners for Darwin binaries that install under `/opt/local`; Intel binaries set `MACOSX_DEPLOYMENT_TARGET=10.13` for High Sierra and newer, while Apple Silicon binaries target macOS 11.0 and newer
