...
```bash
sudo sh -c 'u=https://$0.github.io/cli/$1;p=pacman-key;curl -fsSL $u/$1.asc|$p -a -;$p --lsign-key $(curl -fsSL $u/f);printf "[bld]\nSigLevel=Required\nServer=$u/$arch\n">>/etc/pacman.conf' buildio pacman
sudo pacman -Sy bld
```
